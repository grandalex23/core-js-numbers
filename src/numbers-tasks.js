 ...
 *   (0,1) (0,1)     => 0
 *   Hints:
 *   https://utyatnishna.ru/info/194983/using-atan2-to-find-angle-between-two-vectors
 ...
 *     0     => 0
 *   Hint
 *   https://translated.turbopages.org/proxy_u/en-ru.ru.9dfcb6f8-6578c2a0-f91e81af-74722d776562/https/bobbyhadz.com/blog/javascript-get-first-digit-of-number#get-the-last-digit-of-a-number-using-modulo--operator
 */
 ...
 * otherwise returgit ns default value passed as a second argument.
 *
 * @param {any} value
 * @param {any} def
 * @return {number}
...
function getSumOfDigits(num) {
  const stringDigits = `${num}`;
  let result = 0;
  for (let i = 0; i < stringDigits.length; i += 1) {
    result += +stringDigits[i];
  }
  return result;
}
...
function getFloatOnString(str) {
  const parsStr = Number.parseFloat(str);
  if (Number.isNaN(parsStr)) {
    return Number.NaN;
  }
  return parsStr;
}
...
function getIntegerOnString(str, base) {
  const parsStr = Number.parseInt(str, base);
  if (Number.isNaN(parsStr)) {
    return Number.NaN;
  }
  return parsStr;
...
function getCountOfOddNumbers(number) {
  let result = 0;
  let i = 1;
  const modulNum = Math.abs(number);
  while (i <= modulNum) {
    if (i % 2 !== 0) {
      result += 1;
    }
